...
if [ -d "./public/wp" ]; then DIRS=("public/app/uploads"); fi
if [ -f "./bin/magento" ]; then DIRS=("pub/media"); fi

if [ -f "./please" ]; then
    DIRS=("public/img" "public/assets")
fi

[ -z "${DIRS[*]}" ] && echo "Project not compatible." && exit
...
for DIR in "${DIRS[@]}"; do
    ...
    # Setup SSHFS
    if mount | grep "$REMOTE_SERVER_USER@$REMOTE_SERVER_IP" | grep "$DIR" > /dev/null; then
